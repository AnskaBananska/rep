...
info_for_limma <- info_to_analyse %>%  filter(Sample_status %in% c('NR', 'R'), Cycle %in% c('0', '1')) %>% select(-c('TMT_label', 'Set_nr', 'Replicate', 'Sex', 'Age', 'Cancer', 'TMB'))%>% mutate(Cycle_spelled = if_else(Cycle == 0, "ZERO", "ONE"), .keep = 'unused')
...
info_for_limma_genes_t <- data.frame(cbind(names(info_for_limma_genes), t(info_for_limma_genes))) %>% select(-c('V1'))
...
fit <- lmFit(info_for_limma_genes$MMRN2, design_gene1)
fit2 <- eBayes(fit)
