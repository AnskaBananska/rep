...
### PCA - show replicates
```{r}
pca_w_tripl <-prcomp(ungappy_wide_matrix,scale=TRUE,retx=TRUE) 

results_pca_w_tripl <- pca_w_tripl$x %>%
  as.data.frame() %>%
  rownames_to_column(var = "Sample_name")  %>%
  full_join(Metatable, by = "Sample_name") %>%
  filter(Sample_status %in% c('NR', 'R'))

results_pca_replicates_only <- results_pca_w_tripl %>% filter(Cycle == 0 & (Sample_nr == 2|  Sample_nr == 9) )
...
ggsave(path = "C:/Users/1aria/Desktop/rep/Output/Plots", filename = "PCA_highlight_replicates.png",PCA_w_repl_only,dpi=200, height = 13, width = 13, unit = 'cm')
```
### PCA highlight triplicates
```{r}
# use results_pcs_w_tripl df

results_pca_w_tripl <- results_pca_w_tripl  %>% mutate()

PCA_Tripl<- ggplot(results_pca_w_tripl, aes(x=PC1, y=PC2, color = Sample_status)) + 
  ...
  scale_color_manual(values=c('royalblue4','coral3'), labels = c("Non-responder", "Responder"))+
  stat_ellipse()+
  # geom_text_repel(aes(label = ungappy_wide_matrix_full$Sample_nr))+
...
ggsave(path = "C:/Users/1aria/Desktop/rep/Output/Plots", filename = "PCA_all_points.png",PCA_means_all_points,dpi=200, height = 13, width = 13, unit = 'cm')
...
ggsave(path = "C:/Users/1aria/Desktop/rep/Output/Plots", filename = "PCA_means_clusters_R.png",PCA_means_clusters_R,dpi=200,  height = 10, width = 11, unit = 'cm')
...
ggsave(path = "C:/Users/1aria/Desktop/rep/Output/Plots", filename = "PCA_means_clusters_NR.png",PCA_means_clusters_NR,dpi=200,  height = 10, width = 10, unit = 'cm')
```
...
ungappy_wide_matrix_named <- ungappy_wide_matrix %>% mutate_if(is.character, as.numeric) %>% rownames_to_column(var = 'Sample_name') 
ungappy_wide_matrix_full <- full_join(Metatable, ungappy_wide_matrix_named, by = 'Sample_name') %>%  filter(Sample_status %in% c('NR', 'R'))
PCA_facto <- ungappy_wide_matrix_full %>%  filter(Sample_status %in% c('NR', 'R')) %>% select(c(12, 48)) %>% prcomp(., scale = TRUE)

get_eig(PCA_facto)
fviz_screeplot(PCA_facto, addlabels = TRUE, ylim = c(0, 50))

fviz_pca_ind(PCA_facto, 
            geom.ind ="point",
             habillage=ungappy_wide_matrix_full$Sample_status,
             addEllipses=TRUE,
...
ggplot(ungappy_wide_matrix_full, 
       aes(x = PC1,
           y = PC2,
           color = Sample_status)) +  geom_point() +
geom_text_repel(aes(label = ungappy_wide_matrix_full$Sample_nr))+
  stat_ellipse()+
  theme_bw()
```