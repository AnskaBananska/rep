...
'Check if you have ungappy_wide_matrix_t\_for_PCA.Rda. If so, you can skip the first 0 steps.'
...
```{r}
# PCA object
pca <-prcomp(ungappy_wide_matrix_t_means,scale=TRUE,retx=TRUE)  # full df
...
scree_means <- pca$sdev %>% 
  ...
  ylim(c(0, 100)) +
  ggtitle('Scree plot')+
  geom_text(aes(label=signif(value, digits = 3)*100), nudge_y= 0.01 )+
...
ggsave(path = "C:/Users/1aria/Desktop/rep/Output/Plots", filename = "Scree_plot_PCA_with_means.png",scree_means, height=15,width=17,units="cm",dpi=200)
...
```{r}
### Scree plot from technical reps script
scree_plot_replicates <- pca$sdev %>%
  as_tibble() %>%
  frac_var() %>%
  mutate(Comp = colnames(pca$x)) %>%
  slice(1:9) %>%
