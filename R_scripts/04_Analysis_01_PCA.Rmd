...
PCA_w_repl_v3 <- ggplot(results_pca_replicates_only, aes(x=PC1, y=PC2)) + 
  geom_point(size = 3, color = 'royalblue4')+  #alpha = as.integer(as.factor(Replicate)
  geom_vline(xintercept = 0, linetype=2, col="grey60") +
  geom_hline(yintercept = 0, linetype=2, col="grey60") +
  theme_bw() +
  xlim(c(-60, 60))+
  ylim(c(-50, 50))+
  ggtitle('Replicates only')+
  # labs(shape= "Treatment outcome")+
  geom_text_repel(aes(label = Sample_nr))+
  # scale_shape_manual(values=c(1, 19), labels = c("Non Responder", "Responder"))+
  theme(panel.grid.major = element_blank(), 
...
ggsave(path = "C:/Users/1aria/Desktop/rep/Output/Plots", filename = "PCA_highlight_replicates.png",PCA_w_repl_v3,height=15,width=17,units="cm",dpi=200)
...
### Plot3 layout: Clusters each  infividuals
```{r}
...
### Layout combined
```{r}
layout1 <- ggarrange(PCA_w_repl_v3, PCA_means_SCR, PCA_means_clusters, common.legend = FALSE)

annotate_figure(layout1, top = text_grob("PCA plots", 
               color = "#333333", face = "bold", size = 15))
```
