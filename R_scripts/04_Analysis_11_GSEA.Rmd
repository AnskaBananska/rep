...
GSEA analysis requires a ranked gene list, which contains three features: numeric vector: fold change or other type of numerical variable named vector: every number has a name, the corresponding gene ID sorted vector: number should be sorted in decreasing order
...
# Repeat GSEA, using single timepoint at C3

```{r}
### Create own geneList. (NB. need to convert HUGO symbol to NCBI gene ID) ####
load("C:/Users/1aria/Desktop/rep/Data/Output_data/Diff_expressed/NR_vs_R_C3_results.Rda")
x <- as.data.frame(org.Hs.egSYMBOL2EG)  # map HUGO to NCBI IDs

genes_up_NCBIID <- left_join(NR_vs_R_C3_results, x, by = c("Protein" = "symbol"))  %>% 
  # filter(log2_fc > 0) %>% 
  dplyr::select(gene_id, log2_fc) %>% arrange(., desc(log2_fc))
...
save(GSEA_GO_BP_df, file = "C:/Users/1aria/Desktop/rep/Data/clusterProfilR_GSEA/GSEA_GO_BP_C3only.Rda")
save(GSEA_GO_MF_df, file = "C:/Users/1aria/Desktop/rep/Data/clusterProfilR_GSEA/GSEA_GO_MF.Rda")
save(GSEA_GO_CC_df, file = "C:/Users/1aria/Desktop/rep/Data/clusterProfilR_GSEA/GSEA_GO_CC.Rda")
...
# save
save(GSEA_GO_BP_df_symbols, file = "C:/Users/1aria/Desktop/rep/Data/clusterProfilR_GSEA/GSEA_GO_BP_symbols_C3only.Rda")
save(GSEA_GO_MF_df_symbols, file = "C:/Users/1aria/Desktop/rep/Data/clusterProfilR_GSEA/GSEA_GO_MF_symbols.Rda")
save(GSEA_GO_CC_df_symbols, file = "C:/Users/1aria/Desktop/rep/Data/clusterProfilR_GSEA/GSEA_GO_CC_symbols.Rda")
```
...
GSEA_kegg <- gseKEGG(geneList     = geneList,
               ...
               verbose      = FALSE)
head(kegg)
kegg.df <- as.data.frame(GSEA_kegg)

# Turn kegg genes names from ENTREZ to symbols
GSEA_kegg_symbols <- setReadable(GSEA_kegg, 'org.Hs.eg.db', 'ENTREZID')
kegg_symbols.df <- as.data.frame(GSEA_kegg_symbols)
...
```{r}
dotplot(GSEA_kegg, showCategory=10) + ggtitle("KEGG GSEA - dysregulated at C3") # works

# network plot
kegg_netplot<- cnetplot(GSEA_kegg_symbols, categorySize="p.adjust", foldChange=geneList, circular = FALSE,
                        cex_gene = 0.5, cex_label_gene = 0.5) 
kegg_netplot
ggsave(path = "C:/Users/1aria/Desktop/rep/Output/Plots", filename = "KEGG_netplot.png",kegg_netplot)
...
# repeat GSEA for SCR only

```{r}
load("C:/Users/1aria/Desktop/rep/Data/Output_data/Diff_expressed/NR_vs_R_SCR_results.Rda")
x <- as.data.frame(org.Hs.egSYMBOL2EG)  # map HUGO to NCBI IDs

genes_up_NCBIID <- left_join(NR_vs_R_SCR_results, x, by = c("Protein" = "symbol"))  %>% 
  # filter(log2_fc > 0) %>% 
  dplyr::select(gene_id, log2_fc) %>% arrange(., desc(log2_fc))

# remove where gene ID is na
geneList2 <-  genes_up_NCBIID[,2] %>% as.numeric()
names(geneList2) = as.character(genes_up_NCBIID[,1])
geneList2 = na.omit(geneList2)
geneList2 = sort(geneList2, decreasing = TRUE)
```

```{r}
GSEA_GO_BP <- gseGO(geneList     = geneList2,
              OrgDb        = org.Hs.eg.db,
              ont          = "BP",
              minGSSize    = 10,
              maxGSSize    = 500,
...
GSEA_GO_BP.d <- as.data.frame(GSEA_GO_BP)

def_dotplot2<- dotplot(GSEA_GO_BP, showCategory = 10) + theme(axis.text.y = element_text(size = 9)) + ggtitle('GSEA of GO terms (BP) - SCR dysregulated proteins')+
   theme(plot.title = element_text(size = 12, face = 'bold', hjust = 0.5))
...
dotplot(GSEA_kegg2, showCategory=10) + ggtitle("KEGG GSEA - dysregulated at C3") # works
