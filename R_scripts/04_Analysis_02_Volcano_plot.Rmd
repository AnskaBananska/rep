...
significant_NR_vs_R = NR_vs_R_results %>% na.omit()  %>%  filter(adj_p_value<0.05)  # 19 proteins are signid different expressed
...
volc_NR_vs_R_FC05 <- ggplot(data=NR_vs_R_results_expanded_0.5, aes(x=log2_fc, y=-log10(adj_p_value))) + 
...
# NR vs R - more comparisons

No need to make volcano plots if no differecne found

## Compare only SCR points

No significant differences found

```{r}
# Define conditions -->  colnames of R vs NR
NR_colnames_SCR <- Metatable_means %>% filter(Sample_status == 'NR', Cycle == 0) %>% select(Sample_name) %>% as.list() %>% unlist()
R_colnames_SCR <- Metatable_means %>% filter(Sample_status == 'R', Cycle == 0) %>% select(Sample_name) %>% as.list() %>% unlist()

# Run function
SCR_NR_vs_R_results = as.data.frame(diff_exp(wide_matrix_t_means_no_batch1, colnames_conditionA =NR_colnames_SCR, colnames_conditionB =R_colnames_SCR ))

# Which proteins are diff expressed? (adj p below 0.05?)
significant_SCR_NR_vs_R = SCR_NR_vs_R_results %>% na.omit()  %>%  filter(adj_p_value<0.05)  # 0 proteins are signid different expressed
```
...
```{r}
# Define conditions -->  colnames of R vs NR
NR_colnames_C3 <- Metatable_means %>% filter(Sample_status == 'NR', Cycle == 1) %>% select(Sample_name) %>% as.list() %>% unlist()
R_colnames_C3 <- Metatable_means %>% filter(Sample_status == 'R', Cycle == 1) %>% select(Sample_name) %>% as.list() %>% unlist()

# Run function
C3_NR_vs_R_results = as.data.frame(diff_exp(wide_matrix_t_means_no_batch1, colnames_conditionA =NR_colnames_C3, colnames_conditionB =R_colnames_C3 ))

# Which proteins are diff expressed? (adj p below 0.05?)
significant_C3_NR_vs_R = C3_NR_vs_R_results %>% na.omit()  %>%  filter(adj_p_value<0.05)  # 0 proteins are signid different expressed
```

## Compare SCR + C3

0 significant matches

```{r}
# NR_colnames_C3_SCR <- Metatable_means %>%
#   filter(Sample_status == c('NR'), Cycle == c(1,0)) %>%
#   select(Sample_name) %>%
#   as.list() %>% unlist()

NR_colnames_C3_SCR <- c(
  'NR_7_C3_Set1_tmt16plex_128C',
  'NR_8_SCR_Set1_tmt16plex_130C',
  'NR_9_C3_Set2_tmt16plex_130C',
  'NR_9_SCR_Set2_tmt16plex_mean',
  'NR_10_C3_Set3_tmt16plex_128C',
  'NR_7_SCR_Set1_tmt16plex_127C',
  'NR_8_C3_Set1_tmt16plex_131C',
  'NR_10_SCR_Set3_tmt16plex_127C',
  'NR_11_C3_Set3_tmt16plex_131C',
  'NR_11_SCR_Set3_tmt16plex_130C',
  'NR_12_C3_Set3_tmt16plex_133C',
  'NR_12_SCR_Set3_tmt16plex_132C'
)


R_colnames_C3_SCR <- Metatable_means %>%
  filter(Sample_status == 'R', Cycle == c(1, 0) ) %>%
  select(Sample_name) %>%
  as.list() %>% unlist()

# Run function
C3_SCR_NR_vs_R_results = as.data.frame(diff_exp(wide_matrix_t_means_no_batch1, colnames_conditionA =NR_colnames_C3_SCR, colnames_conditionB =R_colnames_C3_SCR ))

# Which proteins are diff expressed? (adj p below 0.05?)
significant_C3_SCR_NR_vs_R = C3_SCR_NR_vs_R_results %>% na.omit()  %>%  filter(adj_p_value<0.05)  # 0 proteins are signid different expressed
```
...
```{r}
# Define conditions -->  colnames of SCR vs EOT (NR)
NR_colnames_SCR <- Metatable_means %>% filter(Sample_status == 'NR', Cycle == 0, Sample_nr %in% c(7, 8, 10, 11)) %>% select(Sample_name) %>% as.list()  %>%  unlist()
                                              
NR_colnames_EOT <- Metatable_means %>% filter(Sample_status == 'R', Cycle == 1) %>% select(Sample_name) %>% as.list()  %>%  unlist()

# Run function
SCRvsEOTForNR_results = as.data.frame(diff_exp(wide_matrix_t_means_no_batch1, colnames_conditionA =NR_colnames_SCR, colnames_conditionB =NR_colnames_EOT ))

# Manually
results = apply(wide_matrix_t_means_no_batch1, 1, function(x) t.test(x[NR_colnames_SCR], x[NR_colnames_EOT], paired = TRUE, na.action=na.omit))
